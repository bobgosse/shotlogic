...
import { ArrowLeft, Film, Camera, Printer, Download, RefreshCw, FileText, Edit, Save, Grid, ChevronDown, ChevronsDownUp, ChevronsUpDown, Menu, Sparkles, ImageIcon, Palette, X, Check } from "lucide-react";
...
const ProjectDetails = () => {
  ...
  const handleSaveVisualStyle = async () => {
    ...
    try {
      const { error } = await supabase
        .from('projects')
        .update({ visual_style: tempVisualStyle.trim() || null })
        .eq('id', project.id);
  ...
  const handleSaveEdits = async () => {
    ...
    setIsSaving(true);
    try {
      // Update each edited scene
      for (const [sceneId, analysisData] of Object.entries(editedScenes)) {
        const { error } = await supabase
          .from('scenes')
          .update({ 
